...
def loadStats():
    ...
    for line in playerFile: #Check all lines in the file
        if 'High Score: ' in line: #Check if 'High Score: ' is on that line
            highScore = int(line.replace('High Score: ', '')) #Get the int value from that line
        if 'Previous Score: ' in line: #Check if 'Previous Score: ' is on that line
            previousScore = int(line.replace('Previous Score: ', '')) #Get the int value from that line
        if 'Unsuccessful Runs: ' in line: #Check if 'Unsucccessful Runs: ' is on that line
            unsuccessfulRuns = int(line.replace('Unsuccessful Runs: ', '')) #Get the int value from that line
        if 'Successful Runs: ' in line: #Check if 'Successful Runs: ' is on that line
            successfulRuns = int(line.replace('Successful Runs: ', '')) #Get the int value from that line
        if 'Difficulty: ' in line: #Check if 'Difficulty: ' is on that line
            difficulty = int(line.replace('Difficulty: ', '')) #Get the int value from that line
    playerFile.close() #Close the file, prevents unwanted edits to the file

    return highScore, previousScore, unsuccessfulRuns, successfulRuns, difficulty #Return the new values to the program 
...
class Tile():
    ...
    #Draws tile
    def draw(self, screen, player, offset):
...
class Health(Entity):
    ...
    def update(self):
        ...
        if self.hitpoints <= 0:
            #Increasing health
            player.hitpoints += random.randint(9, 15) + (25 * levelCount) + (3 * difficulty)
            ...
            score += 100 + (25 * difficulty) + (5 * levelCount) #Increase score upon collect
...
class Enemy(Entity):
    def __init__(self, x, y, tileType, room):
        ...
        self.defaultActionTimer = 20 + 3 * difficulty
        self.actionTimer = self.defaultActionTimer

        #Set attributes of the enemy according to the level count
        global levelCount
        self.hitpoints = int((4 + difficulty) + (levelCount / 2.5))
        self.power = int(levelCount / 2.5)
        if self.power == 0:
            self.power = 1
        if self.hitpoints == 0:
            self.hitpoints = 1
...
class Player(Entity):
    ...
    def __init__(self, x, y, tileType):
        ...
        self.defaultMoveTimer = 12 + 3 * difficulty
        self.moveTimer = self.defaultMoveTimer
        self.defaultAttackTimer = 16 + 3 * difficulty
        self.attackTimer = self.defaultAttackTimer

        #Redefines hitpoints for the player
        self.maxHitpoints = 100 + (-12 * difficulty) + ((12 + (levelCount*3)) * (levelCount - 1))
        self.hitpoints = self.maxHitpoints
...
class Room():
    ...
    def unlockRoom(self):
        ...
        for i in range(0, int(levelCount/2)):
    ...
    def spawnEnemies(self):
        global levelCount
        number = random.randint(0, 1) + difficulty + int(levelCount / 2)
        if number == 0:
            number = 1
    ...
    def spawnHealth(self):
        ...
        lvlCountNum = int(levelCount/6)
        if lvlCountNum == 0:
            lvlCountNum = 1
        for i in range(0, lvlCountNum):
            if chanceNum >= spawnChance:
                spawnTile = self.getSpawnTile()
...
def drawGrid(screen):
    for row in range(rows):
        for col in range(columns):
            tile = grid[row][col]
            offset = getOffset()
            tile.draw(screen, player, offset)
...
def generateLevel():
    ...
    rows = 15 + 15 * levelCount
    columns = 15 + 15 * levelCount
